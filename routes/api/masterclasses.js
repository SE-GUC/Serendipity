    ...
    masterclasses.forEach(async value => {
       try{
        if(value.id === req.params.id) {
            ...
            data+= `Id: ${value.id}<br>Name: ${value.title}<br>eduOrganisation: ${value.Eduorganizati}<br>duration: ${value.duration}<br>price: ${value.price}<br>description: ${value.description}<br>location: ${value.location}<br>workshops: ${allworkshops}<br>courses: ${value.courseIDs}`;
            // return;
        }
    }catch(error)
    {
